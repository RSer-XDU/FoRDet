...
def eval_map(det_results,
    ...
    for i in range(num_classes):
        ...
        # linewidth = 2
        # import pickle 
        # recall_file = open('recalls.pkl', 'wb')
        # precisions_file = open('precisions.pkl', 'wb')
        # pickle.dump(recalls, recall_file)
        # pickle.dump(precisions, precisions_file)
        ...
        # plt.plot(recalls,precisions,color="red",linewidth=linewidth)
        # plt.xlabel("Recall") #xlabel、ylabel：分别设置X、Y轴的标题文字。
        # plt.ylabel("Precision")
        # plt.title('PRC') # title：设置子图的标题。
        # plt.ylim(0,1.01)# xlim、ylim：分别设置X、Y轴的显示范围。
        # plt.xlim(0,1)
        # plt.legend(loc=3)
