...
from mmdet.core import (AnchorGenerator, soft_anchor_target, weighted_smoothl1,
...
class SOFTITERFRODMSSDHeadbbox(AnchorHead):
    ...
    def forward(self, feats, arm_outs):
        ...
        self.current_iteration =   self.current_iteration + 1.0
    ...
    def loss(self,
        ...
        arm_anchors_list, arm_cls_scores_list = self.get_arm_bboxes(arm_cls_scores, arm_bbox_preds, img_metas)

        iter_factor = min(1, (self.current_iteration / 10000) **2)

        cls_reg_targets = soft_anchor_target(
            arm_anchors_list,
            arm_cls_scores_list,
            valid_flag_list,
            gt_bboxes,
    ...
    def get_arm_bboxes(self, cls_scores, bbox_preds, img_metas, rescale=False):
        ...
        for img_id in range(len(img_metas)):
            ...
            proposals, mlvl_scores = self.get_arm_bboxes_single(cls_scores_list, bbox_pred_list,
                                               mlvl_anchors, img_shape,
                                               scale_factor, rescale)

            result_list.append(proposals)
            arm_cls_scores_list.append(mlvl_scores)
        return result_list, arm_cls_scores_list
    ...
    def get_bboxes(self, arm_cls_scores, arm_bbox_preds, cls_scores, bbox_preds, img_metas, cfg,
        ...
        mlvl_bbox_anchors, mlvl_cls_scores = self.get_arm_bboxes(arm_cls_scores, arm_bbox_preds, img_metas)
        ...
        for img_id in range(len(img_metas)):
            ...
            proposals = self.get_bboxes_single(cls_score_list, bbox_pred_list,
                                               mlvl_bbox_anchors[img_id], img_shape,
    ...
    def get_bboxes_single(self,
        ...
        assert len(cls_scores) == len(bbox_preds) == len(mlvl_bbox_anchors)
        mlvl_bboxes = []
        mlvl_scores = []
        for cls_score, bbox_pred, bbox_anchors in zip(cls_scores, bbox_preds,
                                                 mlvl_bbox_anchors):
            ...
            if nms_pre > 0 and scores.shape[0] > nms_pre:
                ...
                bbox_anchors = bbox_anchors[topk_inds, :]
                bbox_pred = bbox_pred[topk_inds, :]
                scores = scores[topk_inds, :]

            bboxes = delta2bbox(bbox_anchors, bbox_pred, self.target_means,
                                self.target_stds, img_shape)
            mlvl_bboxes.append(bboxes)
            mlvl_scores.append(scores)
