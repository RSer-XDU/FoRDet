...
class AttODMSSDHeadRbbox(AnchorHeadRbbox):
    ...
    def forward(self, feats, arm_outs):
        ...
        for feat, arm_cls_scores, reg_conv, cls_conv in zip(feats, arm_cls_scores,
                                                self.reg_convs, self.cls_convs):
                

                N, _, H, W = arm_cls_scores.size()
                # print(arm_cls_scores.size())

                arm_cls_scores = F.softmax( arm_cls_scores.reshape(N, -1, 2, H, W), dim=2)
                # print(arm_cls_scores.size(), arm_cls_scores[0, :, :, 0:4, 0:4])   
                score_att = arm_cls_scores[:,:,1,:,:]  
                # print(score_att.size()) 
                # print(score_att[0, :, 0:4, 0:4])
                score_att = torch.unsqueeze(torch.mean(score_att, dim=1), dim=1)

                feat = feat + feat * score_att
