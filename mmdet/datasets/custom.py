...
from .hbb_rotate_aug import HBBRotateAugmentation
from .rotate_aug import RotateTestAugmentation
from .mask_extra_aug import MaskExtraAugmentation
...
def filter (img, boxes, masks, labels, filename,method=None):
        ...
        if filter_flag == 1:
            ...
            return img, np.zeros((0,4), dtype=np.float32).tolist(), [], np.array([], dtype=np.int64), filter_flag
        else:
            ...
            return img, boxes, masks, labels, filter_flag
...
class CustomDataset(Dataset):
    ...
    def __init__(self,
        ...
        if extra_aug is not None:
            if self.with_mask:
                print('---------------------MaskExtraAugmentation-----------------')
                self.extra_aug = MaskExtraAugmentation(**extra_aug)
            else:
        ...
        if rotate_aug is not None:
            if self.with_mask:
                ...
                print('--------------HBBRotateAugmentation------------------')
                self.rotate_aug = HBBRotateAugmentation(self.CLASSES, **rotate_aug)
        else:
    ...
    def prepare_train_img(self, idx):
        ...
        # print('img_name', img_info['filename'])
        ...
        if len(gt_bboxes) == 0:
            return None
        # from mmdet.core import mask2poly
        # polys = mask2poly(gt_masks)
        # # print(polys)
        # # print(polys.shape)
        # mmcv.imshow_bboxes(img,polys)
        # extra augmentation
        if self.extra_aug is not None:
            if self.with_mask:
                img, gt_bboxes, gt_masks, gt_labels = self.extra_aug(img, gt_bboxes, 
        ...
        if self.rotate_aug is not None:
            ...
            # only support mask now, TODO: support none mask version
                if self.with_mask:
                    ...
                    img, gt_bboxes, gt_labels = self.rotate_aug(img, gt_bboxes,
                                                                      gt_labels, img_info['filename'])
            gt_bboxes = np.array(gt_bboxes).astype(np.float32)
            # skip the image if there is no valid gt bbox
            if len(gt_bboxes) == 0:
                return None
        ...
        if self.with_mask:
            ...
            _, gt_bboxes, _, _, filter_flag = filter(img, gt_bboxes,
                                                                      gt_masks, gt_labels, img_info['filename'])
            gt_bboxes = np.array(gt_bboxes).astype(np.float32)
            # # skip the image if there is no valid gt bbox
            if filter_flag == 1:
        ...
        # img_show = mmcv.imnormalize(img_show, [123.675, 116.28, 103.53], [1, 1, 1])
        # print(img_show.shape)
        # mmcv.imshow_bboxes(img_show,polys)
        ...
        # # print(polys)
        # # print(polys.shape)
        # mmcv.imshow_bboxes(np.transpose(img, [1,2,0]), polys)
        # # assert 2==1
