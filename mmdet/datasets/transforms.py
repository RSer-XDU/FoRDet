...
class MaskTransform(object):
    ...
    def __call__(self, masks, pad_shape, scale_factor, flip=False):
        ...
        if isinstance(scale_factor, float):
            ...
            # print('shape', masks_np.shape)
            w, h = masks_np.shape[1:]
            # print('w,h', w,h)
            scale_factor = (int(round(w * scale_factor[1])), int(round(h *scale_factor[0])))
            # print('mask-transfrom', type(scale_factor), scale_factor)
        # assert 2==1
            
        masks = [
            mmcv.imrescale(mask, scale_factor, interpolation='nearest')
            # mmcv.imresize(mask, scale_factor)
            for mask in masks
        ]
        if flip:
